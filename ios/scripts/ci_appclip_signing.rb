#!/usr/bin/env ruby
# Switches the Makon3DAppClip target to manual Distribution signing for CI.
#
# The clip target uses Automatic signing for local development, but the
# TestFlight workflow signs manually with pre-installed profiles. Runner's
# manual-signing overrides are appended to ios/Flutter/Release.xcconfig,
# which the clip does not include, so this script writes the equivalent
