...
usage() {
  cat <<'EOF'
Usage: tool/release_ios_tag.sh [--bump {build|patch|minor|major}] [--commit]

Creates and pushes an ios-<version> release tag from pubspec.yaml
"version: x.y.z+build" (converts '+' -> '-' for the git tag).

Pushing the tag starts .github/workflows/release-testflight.yml.

Options:
  --bump <type>   Bump version first using tool/bump_version.sh
  --commit        Commit the version bump before tagging (also pushes the bump)
EOF
}
...
echo -e "Tagging ios-${C_YELLOW}${VERSION_SEMVER}${C_RESET}-${C_GREEN}${VERSION_BUILD}${C_RESET} -> ${SHA}"
git tag "${IOS_TAG}"
